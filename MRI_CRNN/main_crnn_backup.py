...
if __name__ == '__main__':
    ...
    train, validate, test = create_dummy_data() 
        ...
        # train: 20,30,256,32 np.array
        # validate: 2,30,256,32 np.array
        # test: 2,30,256,32 np.array
        # undersampling the original data to obtain patches
    ...
    for epoch in range(num_epoch):
        ...
        for im in iterate_minibatch(train, batch_size, shuffle=True):
            ...
            im_und, k_und, mask, im_gnd = prep_input(im, acc)
                # complex -> 2 channels [,2, ...]
                # im_und: image space undersampled data (image -> fft -> mask -> ifft) torch.Size([1, 2, 256, 32, 30])
                # k_und: k-space undersampled data (image -> fft -> mask) torch.Size([1, 2, 256, 32, 30])
                # im_gnd: ground truth data torch.Size([1, 2, 256, 32, 30])
