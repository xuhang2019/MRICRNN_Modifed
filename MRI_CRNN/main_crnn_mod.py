...
from cascadenet_pytorch.dnn_io import c2r
...
def prep_input(im, mode):
    ...
    if mode == 'train':
        mask = load_mat('./data/cine_lax_mask.mat')
        mask = np.array(mask[..., 0:68, :])
    if mode == 'val':
        mask = load_mat('./data/cine_lax_val_mask.mat')
        mask = np.array(mask[..., 0:68, :])
    if mode == 'test':
        mask = load_mat('./data/cine_lax_test_mask.mat')
        mask = np.array(mask[..., 0:68, :])
    im = np.fft.ifft2(im)
...
# ====================
tr,val,test = create_dummy_data()
print(tr.shape,'trrrrrrrrrrrrrrrrrr')

def create_data():


    data_train = load_mat('D:\SingleCoil\Cine\TrainingSet\FullSample\P001\\cine_lax.mat')
    data_train = to_complex(data_train)
    data_val = load_mat('./data/cine_lax_val.mat')
    data_val = to_complex(data_val)
    data_test = load_mat('./data/cine_lax_test.mat')
    data_test = to_complex(data_test)

    ny = 204
    ny_red = 6
    sl = ny//ny_red

    # data_train = np.transpose(data_train, (0, 1, 3, 2))
    train = np.array(data_train[..., 0:68,:])
    validate = np.array(data_val[..., 0:68,:])
    test = np.array(data_test[..., 0:68,:])

    return train, validate, test

def to_complex(data):
    data_c = np.ones(data.shape, dtype='complex64')
    datar = data['real']
    datai = data['imag']
        ...
        for y in range(data.shape[1]):
            for i in range(data.shape[2]):
                for x in range(data.shape[3]):
                    data_c[j,y,i, x] = complex(datar[j,y,i,x],datai[j,y,i,x])
...
if __name__ == '__main__':
    ...
    cuda = False if torch.cuda.is_available() else True
    ...
    for epoch in range(num_epoch):
        ...
        for im in iterate_minibatch(train, batch_size, shuffle=True):
            ...
            im_und, k_und, mask, im_gnd = prep_input(im, 'train')
            ...
            print(im_u.shape, rec.shape, gnd.shape, '1111')
        ...
        for im in iterate_minibatch(validate, batch_size, shuffle=False):
            im_und, k_und, mask, im_gnd = prep_input(im, 'val')
            with torch.no_grad():
                im_u = Variable(im_und.type(Tensor))
                k_u = Variable(k_und.type(Tensor))
                mask = Variable(mask.type(Tensor))
        ...
        for im in iterate_minibatch(test, batch_size, shuffle=False):
            im_und, k_und, mask, im_gnd = prep_input(im, 'test')
            with torch.no_grad():
                im_u = Variable(im_und.type(Tensor))
                k_u = Variable(k_und.type(Tensor))
                mask = Variable(mask.type(Tensor))
