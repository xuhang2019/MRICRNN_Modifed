...
class DataConsistencyInKspace(nn.Module):
    ...
    def fft2c(self, x):
        dim = (-2, -1)
        res = torch.fft.fftshift(torch.fft.fft2(torch.fft.ifftshift(x, dim=dim), norm=self.norm), dim=dim) 
        return res
    
    def ifft2c(self, x):
        dim = (-2, -1)
        res = torch.fft.fftshift(torch.fft.ifft2(torch.fft.ifftshift(x, dim=dim), norm=self.norm), dim=dim)
        return res
    ...
    def perform(self, x, k0, mask):
        ...
        # sort to a complex way for fft operation
        
        # fft2c mask
        x = torch.complex(x[...,0],x[...,1])
        
        # here, modify fft2 to fft2c
        k = self.fft2c(x) 
        k_real = k.real
        k_imag = k.imag
        k = torch.stack((k_real,k_imag),-1)
          
        out = data_consistency(k, k0, mask, self.noise_lvl)
        out = torch.complex(out[...,0],out[...,1])
        x_res = self.ifft2c(out)        
        x_res = torch.stack((x_res.real,x_res.imag),-1)
