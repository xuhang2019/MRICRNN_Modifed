...
def load_mat(file_path:str, complex=False):
    ...
    Args:
        ...
        complex: if True, return a complex ndarray (for data), 
                else return a real ndarray (for mask)
    """
...
def normalized(complex_data_im, m_min=0, m_max=0.004):
    ...
    m_max, m_min = magnitude.max(), magnitude.min()
...
class KCrop:
    ...
    def __init__(self, only_gt=True):
        self.m_min = 0
        self.m_max = 0.004
        self.only_gt = only_gt
        
    def normalized(self, complex_data_im, m_min=0, m_max=0.004):
        complex_data_im = crop_cmrx(complex_data_im)
        phase = np.angle(complex_data_im)
        magnitude = np.abs(complex_data_im)
    ...
    def __call__(self, k_space, is_mask=False):
        if not is_mask:
            ...
            im_complex = ifft2c(k_complex) # get image space
            # nd.array
            im_crop_complex = self.normalized(im_complex) # crop image spaces
            
            return im_crop_complex, fft2c(im_crop_complex)
...
def only_gt_x(im_y, mask):
    """
        Given a ground truth image and a mask, return the undersampled image and k-space. (Only for only_gt mode)
    """
    im_x = im_y * mask
    k_x = fft2c(im_x)
    return im_x, k_x    
